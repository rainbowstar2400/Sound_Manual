...
※TipsのGoogleDriveにも置いてあるので、そちらからダウンロードでもOK（`客入れ.mp3`）
[TipsのGoogleDrive「音響」フォルダ](https://drive.google.com/drive/u/0/folders/1Wf5QIO4ESNrqhnME6_MwYCWBmDZwO3yI)
...
#### ONJOが重いときは
重い・動作が安定しない場合は、まず以下の点を確認すること。

- 不必要なウィンドウ・ソフトウェアが開かれていないか
  ウィンドウを最小化しただけだと、裏で動いているので重くなる原因に。できる限りすべて閉じておく。
  バックグラウンドで動いているソフトもできるだけ落としておこう。右下の∧から、バックグラウンドで動作中のソフト一覧が表示される。大抵の場合落としても大丈夫なので気にせず落とそう。

- 電源に接続しているか（充電されているか）
  単純に電力を消費するのでコードを繋いでおこう。コンセントがない場合などを除いてできるだけ普段からも繋いで使用する方が良い。

それでも重いようであれば、一旦ONJOの再起動、またはPC本体の再起動を試す。
試してもダメな場合は先輩方や詳しい人間に相談してください。多分きっと何かある。
